# 3.1 Create a New Agent in Agentforce Builder
...
4. In the **Name your agent** modal, enter:
   ...
   | Field | Value |
   | --- | --- |
   | Agent Name | EDB Account Research Agent |
   | API Name | EDB_Account_Research_Agent |
...
## 3.1.3 Explore Canvas & Agent Script
...
4. Click **System** in the explorer to view the agent-level **Instructions**, **Welcome
   ...
   > Hi, I'm the EDB Account Research Agent. Which company would you like me to look into?

   ![Change the welcome message](/screenshots/agent-welcome-message.png)

5. Click **Save**.

## 3.1.4 Understand the Agent Router
...
2. Click **Permission Set Assignments > Edit Assignments**, and add:
   - **Agentforce Agent EDB_Account_Research_Agent** (the permission set generated for
     this agent)
3. Click **Save**.

## 3.1.8 Grant Agent Access on the Permission Set

1. From **Setup**, search **Permission Sets** in Quick Find and select **Permission
   Sets**.
2. Open the **Agentforce Agent EDB_Account_Research_Agent** permission set.

   ![The agent permission set](/screenshots/agent-permission-set.png)

3. Click **Agent Access**.
4. Click **Edit**.
5. Add the **EDB Account Research Agent** agent and click **Save**.
...
We'll come back here after **Exercise 3.2** to ask account-specific questions once the
...
Next, you'll add a subagent and actions so it can reason over real account data. 👇
:::
